...
describe("RoomView", () => {
    ...
    describe("when rendering a DM room with a single third-party invite", () => {
        ...
        it("should render the »waiting for third-party« view", () => {
            ...
            expect(screen.getByText(`Waiting for users to join ${brand}`)).toBeInTheDocument();
            expect(
                screen.getByText(
                    `Once invited users have joined ${brand}, you will be able to chat and the room will be end-to-end encrypted`,
                ),
            ).toBeInTheDocument();
    ...
    describe("chat effects", () => {
        ...
        it("fires for a non-muted room even when the message doesn't bump the notification badge", async () => {
            ...
            // reasoning.
            room.getUnreadNotificationCount = vi.fn().mockReturnValue(0);
            room.getRoomUnreadNotificationCount = vi.fn().mockReturnValue(0);
