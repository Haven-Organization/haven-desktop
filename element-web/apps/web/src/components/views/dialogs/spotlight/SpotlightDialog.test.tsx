...
describe("Spotlight Dialog", () => {
    ...
    describe("knock rooms", () => {
        ...
        // Haven: ask-to-join is always on (the feature_ask_to_join flag no longer gates it), so there's
        // no "disabled" variant to cover here.
        describe("knock rooms", () => {
            beforeEach(async () => {
                vi.spyOn(mockedClient, "getRoom").mockReturnValue(null);
                // Haven: the room isn't known locally, so it's looked up (MSC3266 room summary) to check it isn't a
                // Social profile/group room before the normal handling runs - an ordinary room type here.
                mockedClient.getRoomSummary = vi.fn().mockResolvedValue({ room_id: knockRoom.room_id });
                ...
                // Haven: a room this client doesn't know is first looked up (tryRouteSocialRoom, to see whether it
                // belongs in the Social app) before the normal view/ask-to-join handling runs, so it's async.
                await flushPromisesWithFakeTimers();
            });
