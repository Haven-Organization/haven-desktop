...
//
// Kept in its own file rather than folded into createRoom.test.ts - that file's own "sets up Jitsi
// video rooms correctly" test relies on vi.spyOn-ing the real ./models/Call implementation, which
// this file's own vi.mock of that same module would otherwise clobber for every test in the file
// (vi.mock hoists and applies module-wide, not just within one describe block).
