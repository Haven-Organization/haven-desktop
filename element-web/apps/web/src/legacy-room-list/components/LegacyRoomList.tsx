...
import { _t, _td } from "../../languageHandler.ts";
...
import { type ITagMap } from "../stores/algorithms/models.ts";
import { DefaultTagID, type TagID } from "../../stores/room-list-v3/skip-list/tag";
import { UPDATE_EVENT } from "../../stores/AsyncStore.ts";
import RoomListStore, { LISTS_UPDATE_EVENT } from "../stores/RoomListStore.ts";
import {
    isMetaSpace,
    type ISuggestedRoom,
    MetaSpace,
...
const DmAuxButton: React.FC<IAuxButtonProps> = ({ tabIndex, dispatcher = defaultDispatcher }) => {
    ...
    const activeSpace = useEventEmitterState(SDKContextClass.instance.spaceStore, UPDATE_SELECTED_SPACE, () => {
        return SDKContextClass.instance.spaceStore.activeSpaceRoom;
    });
...
const UntaggedAuxButton: React.FC<IAuxButtonProps> = ({ tabIndex }) => {
    const [menuDisplayed, handle, openMenu, closeMenu] = useContextMenu<HTMLDivElement>();
    const activeSpace = useEventEmitterState<Room | null>(SDKContextClass.instance.spaceStore, UPDATE_SELECTED_SPACE, () => {
        return SDKContextClass.instance.spaceStore.activeSpaceRoom;
    });
...
export default class LegacyRoomList extends React.PureComponent<IProps, IState> {
    ...
    public constructor(props: IProps) {
        ...
        this.state = {
            sublists: {},
            suggestedRooms: SDKContextClass.instance.spaceStore.suggestedRooms,
        };
    }
    ...
    public componentDidMount(): void {
        ...
        SDKContextClass.instance.spaceStore.on(UPDATE_SUGGESTED_ROOMS, this.updateSuggestedRooms);
    ...
    public componentWillUnmount(): void {
        ...
        SDKContextClass.instance.spaceStore.off(UPDATE_SUGGESTED_ROOMS, this.updateSuggestedRooms);
