...
const LegacyRoomListHeader: React.FC<IProps> = ({ onVisibilityChange }) => {
    ...
    const [spaceKey, activeSpace] = useEventEmitterState<[SpaceKey, Room | null]>(
        SDKContextClass.instance.spaceStore,
        UPDATE_SELECTED_SPACE,
        () => [SDKContextClass.instance.spaceStore.activeSpace, SDKContextClass.instance.spaceStore.activeSpaceRoom],
    );
    const allRoomsInHome = useEventEmitterState(SDKContextClass.instance.spaceStore, UPDATE_HOME_BEHAVIOUR, () => {
        return SDKContextClass.instance.spaceStore.allRoomsInHome;
    });
