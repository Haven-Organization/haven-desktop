...
import { render, screen, act, type RenderResult } from "test-utils-rtl";
import {
    type MatrixClient,
...
// vitest.config.ts always resolves the "legacy-room-list" specifier to the stub (LEGACY_ROOM_LIST_AVAILABLE
// false). MessagePreviewStore only adds each thread's last reply to a room's preview while the legacy room list
// is in use (see its generatePreview), so these tests - which import the real RoomTile by relative path - need
// it to report that it's available.
...
describe("RoomTile", () => {
    ...
    describe("when message previews are enabled", () => {
        ...
        describe("and there is a message in a thread", () => {
            beforeEach(() => {
                // The thread reply only shows up in the preview while the old room list is switched on.
                const getValue = SettingsStore.getValue.bind(SettingsStore);
                vi.spyOn(SettingsStore, "getValue").mockImplementation((name, ...args) =>
                    name === "Haven.useOldRoomList" ? true : getValue(name, ...args),
                );
