...
import { ListAlgorithm, SortAlgorithm } from "./algorithms/models";
import { OrderedDefaultTagIDs, RoomUpdateCause } from "./models";
import RoomListStore, { RoomListStoreClass } from "./RoomListStore";
...
describe("RoomListStore", () => {
    ...
    client.getRoom = vi.fn().mockImplementation((roomId) => {
        switch (roomId) {
            case newRoomId:
    ...
    function createStore(): { store: RoomListStoreClass; handleRoomUpdate: Mock } {
        const fakeDispatcher = { register: vi.fn() } as unknown as MatrixDispatcher;
    ...
    it("Lists all rooms that the client says are visible", () => {
        ...
        client.getVisibleRooms = vi.fn().mockReturnValue([room1, room2, room3]);
    ...
    it("Watches the feature flag setting", () => {
        vi.spyOn(SettingsStore, "watchSetting").mockReturnValue("dyn_pred_ref");
        vi.spyOn(SettingsStore, "unwatchSetting");
    ...
    it("Regenerates all lists when the feature flag is set", () => {
        ...
        vi.spyOn(SettingsStore, "getValue").mockImplementation(() => featureFlagValue);

        let watchCallback: CallbackFn<any> | undefined;
        vi.spyOn(SettingsStore, "watchSetting").mockImplementation((_settingName, _roomId, callbackFn) => {
    ...
    describe("room updates", () => {
        ...
        describe("push rules updates", () => {
            ...
            it("triggers a room update when room mutes have changed", async () => {
                ...
                const algorithmSpy = vi.spyOn(store.algorithm, "handleRoomUpdate").mockReturnValue(undefined);
