...
export default class SpaceStore extends AsyncStoreWithClient<EmptyObject> {
     ...
     *
     * Haven: People and Favourites are only shown when the old room list is in use - see
     * legacy-room-list/index.ts's own doc for why that's still an option at all.
     * @private
     */
    private get metaSpaceOrder(): MetaSpace[] {
        if (!LEGACY_ROOM_LIST_AVAILABLE || !SettingsStore.getValue("Haven.useOldRoomList")) {
