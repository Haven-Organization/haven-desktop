...
import { type CategoryKey, type Category, categoryName } from "./EmojiPicker";
...
export const Tabs: React.FC<Props> = ({
    ...
    // Haven: was gated on `!tabRefs.current.recent` - a one-time-population guard that assumed
    // "recent" is always among `categories`, which sticker mode's own category list never includes
    // (see EmojiPicker.tsx's own mode doc). Keyed on the actual category id set instead, so this
    // still only (re-)populates when the categories genuinely changed, in either mode.
    ...
    const populatedKeyRef = useRef<string>("");
    if (populatedKeyRef.current !== categoryIdsKey) {
        ...
        <nav className={styles.header} role="tablist" aria-label={_t("emoji|categories")} onKeyDown={onKeyDown}>
            {categories.map((category) => {
                ...
                return (
                    ...
                    >
                        {category.iconUrl ? (
                            <img src={category.iconUrl} alt="" className={styles.anchorIcon} />
                        ) : (
                            category.emoji
                        )}
                    </button>
